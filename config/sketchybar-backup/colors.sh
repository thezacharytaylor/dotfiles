#!/bin/bash

# Rose Pine Moon | Rose Pine Dawn | Catuppuccin | Witch Hazel

# Color Palette
# Rose Pine Moon
# ===============================
# export BLACK=0xff2a283e
# export WHITE=0xffe0def4 # 0xff575279 # 0xffcad3f5
# export RED=0xffeb6f92 # 0xffb4637a # 0xffed8796
# export GREEN=0xff9ccfd8 # 0xff56949f # 0xffa6da95
# export BLUE=0xff3e8fb0 # 0xff286983 # 0xff8aadf4
# export YELLOW=0xfff6c177 # 0xffea9d34 # 0xffeed49f
# export ORANGE=0xffea9a97 # 0xffd7827e # 0xfff5a97f
# export MAGENTA=0xffc4a7e7 # 0xff907aa9 # 0xffc6a0f6
# export GREY=0xff636a86 # 0xff9893a5 # 0xff939ab7

# Rose Pine Dawn
# ===============================
# export BLACK=0xfff4ede8
# export WHITE=0xff575279
# export RED=0xffb4637a
# export GREEN=0xff56949f
# export BLUE=0xff286983
# export YELLOW=0xffea9d34
# export ORANGE=0xffd7827e
# export MAGENTA=0xff907aa9
# export GREY=0xff9893a5

# Catuppuccin
# ===============================
# export BLACK=0xff1e1e2e
# export WHITE=0xffcdd6f4
# export PINK=0xfff5c2e7
# export RED=0xfff38ba8
# export GREEN=0xffa6e3a1
# export BLUE=0xff89b4fa
# export YELLOW=0xfff9e2af
# export ORANGE=0xfff5a97f
# export MAGENTA=0xfffab387
# export LAVENDER=0xffb4befe
# export GREY=0xff939ab7

# Catuppuccin Frappe
# ===============================
# export BLACK=0xff303446
# export WHITE=0xffc6d0f5
# export PINK=0xfff4b8e4
# export RED=0xffe78284
# export GREEN=0xffa6d189
# export BLUE=0xff8caaee
# export YELLOW=0xffe5c890
# export ORANGE=0xffef9f76
# export MAGENTA=0xffca9ee6
# export LAVENDER=0xffbabbf1
# export GREY=0xff949cbb

# Witch Hazel
# ===============================
export BLACK=0xff282634
export WHITE=0xffF8F8F0
export PINK=0xffFFB8D1
export RED=0xffDC7070
export GREEN=0xff81FFBE
export BLUE=0xff1Bc5E0
export YELLOW=0xffFFF9A3
export ORANGE=0xff894E63
export MAGENTA=0xffAE81FF
export LAVENDER=0xffDCC8FF
export GREY=0xffBFBFBF

# General Palette Colors
# ===============================
export TRANSPARENT=0x00000000

# Bar Palette Colors
# Rose Pine Moon
# ===============================
# export BAR_COLOR=0xff232136 # 0xfffaf4ed # 0xff1e1e2e
# export BAR_BORDER_COLOR=0xffea9a97 # 0xffd7827e # 0xff494d64 #0xa024273a
# export BACKGROUND_1=0xff2a273 # 0xfffffaf3 # 0x603c3e4f
# export BACKGROUND_2=0xff2a283e # 0xfff4ede8 # 0x60494d64
# export POPUP_BACKGROUND_COLOR=0xff393552 # 0xfff2e9e1 # 0xff1e1e2e

# Rose Pine Dawn
# ===============================
# export BAR_COLOR=0xfffaf4ed # 0xff1e1e2e
# export BAR_BORDER_COLOR=0xffd7827e # 0xff494d64 #0xa024273a
# export BACKGROUND_1=0xfffffaf3 # 0x603c3e4f
# export BACKGROUND_2=0xfff4ede8 # 0x60494d64
# export POPUP_BACKGROUND_COLOR=0xfff2e9e1 # 0xff1e1e2e

# Catuppuccin
# ===============================
# export BAR_COLOR=0xff1e1e2e
# export BAR_BORDER_COLOR=0xffcba6f7 #0xa024273a
# export BACKGROUND_1=0x603c3e4f
# export BACKGROUND_2=0x60494d64
# export POPUP_BACKGROUND_COLOR=0xff1e1e2e

# Catuppuccin
# ===============================
# export BAR_COLOR=$BLACK
# export BAR_BORDER_COLOR=$MAGENTA
# export BACKGROUND_1=0xff414559
# export BACKGROUND_2=0xff51576d
# export POPUP_BACKGROUND_COLOR=$BLACK

# Witch Hazel Hypercolor
# ===============================
export BAR_COLOR=$BLACK
export BAR_BORDER_COLOR=$MAGENTA
export BACKGROUND_1=0xff3B364E
export BACKGROUND_2=0xff282634
export POPUP_BACKGROUND_COLOR=$BLACK

# General Bar Colors
# ===============================
export ICON_COLOR=$WHITE # Color of all icons
export LABEL_COLOR=$WHITE # Color of all labels
export POPUP_BORDER_COLOR=$WHITE
export SHADOW_COLOR=$BLACK
